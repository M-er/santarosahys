...
class ProductMapper extends Mapper
{
	...
	public function hojeador($request, $response, array $args)
	{
		...
		if ($datos['texto']) {
			$query = 'SELECT SQL_CALC_FOUND_ROWS producto.*, nombuser from producto left join user on iduser = user_iduser where (descripcion like "%'.$datos['texto'].'%" or nombprod like "%'.$datos['texto'].'%") order by '.$ordenamiento.' limit '.$datos['desde'].','.$datos['cuantos'].';';
			$todas = $db->getAllRecords($query);
		} else {
			$query = 'SELECT SQL_CALC_FOUND_ROWS producto.*, nombuser from producto left join user on iduser = user_iduser order by '.$ordenamiento.' limit '.$datos['desde'].','.$datos['cuantos'].';';
			$todas = $db->getAllRecords($query);
		}

...
class CursoMapper extends Mapper
{
	...
	public function hojeador($request, $response, array $args)
	{
		...
		if ($datos['texto']) {
			$query = 'SELECT SQL_CALC_FOUND_ROWS curso.*, nombuser from curso left join user on iduser = user_iduser where (titulo like "%'.$datos['texto'].'%" or curso.path like "%'.$datos['texto'].'%" or nombuser like "%'.$datos['texto'].'%" ) order by '.$ordenamiento.' limit '.$datos['desde'].','.$datos['cuantos'].';';
			$todas = $db->getAllRecords($query);
		} else {
			$query = 'SELECT SQL_CALC_FOUND_ROWS curso.*, nombuser from curso left join user on iduser = user_iduser order by '.$ordenamiento.' limit '.$datos['desde'].','.$datos['cuantos'].';';
			$todas = $db->getAllRecords($query);
		}
